...
- [x] Task: Create integration tests for plugin template generation
    - [ ] Add test cases in `tests/test_generator.py` that check for nested folder creation and template rendering from plugins.
- [x] Task: Refactor `LitestarGenerator` in `src/Litestar/generator.py`
...
- [x] Task: Verify Phase 2 passing
    - [ ] Run `pytest tests/test_generator.py`.
- [x] Task: Checkpoint - Verify Phase 2 complete and create recovery point [checkpoint: cda42b7]
- [x] Task: Flow - User Manual Verification 'Litestar Generator Logic' (Protocol in workflow.md)
